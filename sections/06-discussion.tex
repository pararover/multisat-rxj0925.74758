\section{Discussion and Conclusions}
	%As per current literature on spectral fitting of the SSS \source, Bearda \textit{et al.} (2002) \cite{beardaChandra2002AA} and Motch \textit{et al.} (2002) \cite{motchXmmNewton2002AA} had arrived at the conclusion that its spectra cannot be reproduced by NLTE model atmospheres. %\cite{beardaChandra2002AA,motchXmmNewton2002AA}.
	%Previously, Hartmann \textit{et al.} (1999) \cite{hartmann1999constraining} had applied non-local thermodynamic equilibrium (NLTE) models, which included metal line opacities, to the spectrum extracted from the observations of \source\ by the LECS instrument on-board \textit{BeppoSAX} observatory on 25--26 January 1997, %\cite{hartmann1999constraining}
...
%    \end{figure*}
	...
	%\input{sections/06-discussion-NLTE-pureH}
    ...
    In the current work, we are trying to understand the supersoft continuum spectrum of \source, across observations from 1994 to 2019. The idea is to develop a robust model that explains its supersoft spectrum over different time scales and different observing instruments. %The best-fit model consists of XSPEC model components which consider various factors affecting the observed spectrum:
%    \begin{enumerate}[a)]
...
%   	\end{enumerate}
    ...
    Initially, a simple blackbody model failed to fit the spectrum. Blackbodies emit a characteristic spectrum based on temperature, and it seems this does not accurately represent the complex emission from \source. Then the supersoft X-ray emission of \source\ was modelled using an NLTE stellar atmosphere model \cite{werner1999classical}. Some of these are made available as tables in appropriate FITS format to be used with the XSPEC table model component \texttt{atable} \cite{rauch2003grid,rauch2010nlte}. These models, which are essentially pre-calculated grids, vary from one another in terms of the elemental abundances, the effective temperature $T_\text{eff}$ and the effective gravity $\log{g}$ (where the gravitational acceleration $g$ is expressed in cgs units). Choosing a particular FITS file fixed the specific elemental abundance and effective gravity. Then the fitting of the model to the data is performed by using the effective temperature $T_\text{eff}$ and the redshift $z$ as parameters.
    
    During the analysis of the spectra from all six observations of \source, we found that only an %NLTE model with a pure hydrogen atmosphere$^{27}$
    ...
    There are a couple of caveats to drawing such an inference regarding \source:
    \begin{enumerate}
    ...
    %In this regard, we would like to report that we have had some degree of success in obtaining a good fit with $\chi^2_\text{reduced}=1.48$ using only the data from the second order spectrum of RGS data from \textit{XMM-Newton} for the same observation ID mentioned in the present work. The spectral model that has been used to achieve this, however, requires a combination of several components. We are currently working on developing a table model for XSPEC that would make use of synthetic spectra obtained from a model atmosphere which is computed from customised metal opacities, rather than a pure-H NLTE model as used in the present work. Simultaneously, we propose the inclusion of a stellar wind model that would be able to model the P Cygni profiles which have been observed in the spectrum of \source, for instance that in the Ly $\alpha$ O VIII line at 18.97 \AA\ due to stellar wind velocities $\sim 2000$ km/s. We hope that such a phenomenological approach would lead us towards the interpretation of data from the rich high-resolution grating spectra.
